import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { apiClient } from '../../lib/api/client';
import { formatINR } from '../../lib/formatters';

export const AdminDisputesPage: React.FC = () => {
  const [disputes, setDisputes] = useState<any[]>([]);
  const [selectedStatus, setSelectedStatus] = useState<string>('ALL');
  const [isLoading, setIsLoading] = useState(true);
  const [activeDispute, setActiveDispute] = useState<any | null>(null);
  const [resolutionStatus, setResolutionStatus] = useState<'RESOLVED_BUYER_REFUND' | 'RESOLVED_SELLER_PAYOUT' | 'UNDER_REVIEW' | 'REJECTED'>('RESOLVED_BUYER_REFUND');
  const [resolutionNotes, setResolutionNotes] = useState('');
  const [isSubmittingResolution, setIsSubmittingResolution] = useState(false);
  const [actionSuccess, setActionSuccess] = useState<string | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);

  async function loadDisputes() {
    try {
      setIsLoading(true);
      const url = selectedStatus === 'ALL' ? '/admin/disputes' : `/admin/disputes?status=${selectedStatus}`;
      const res: any = await apiClient.get(url);
      setDisputes(res.data.disputes || []);
    } catch (err) {
      // Ignore
    } finally {
      setIsLoading(false);
    }
  }

  useEffect(() => {
    loadDisputes();
  }, [selectedStatus]);

  const handleOpenArbitration = (dispute: any) => {
    setActiveDispute(dispute);
    setResolutionStatus('RESOLVED_BUYER_REFUND');
    setResolutionNotes('');
    setActionError(null);
  };

  const handleResolveDispute = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeDispute) return;

    try {
      setIsSubmittingResolution(true);
      setActionError(null);

      await apiClient.patch(`/admin/disputes/${activeDispute.id}/resolve`, {
        status: resolutionStatus,
        resolutionNotes: resolutionNotes.trim() || undefined,
      });

      setActionSuccess(`Dispute #${activeDispute.id.slice(0, 8)} successfully updated to ${resolutionStatus.replace(/_/g, ' ')}.`);
      setActiveDispute(null);
      await loadDisputes();
    } catch (err: any) {
      setActionError(err.message || 'Failed to submit dispute resolution.');
    } finally {
      setIsSubmittingResolution(false);
    }
  };

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'OPENED':
        return (
          <span className="px-3 py-1 rounded-full bg-[#9B5C52]/15 text-[#9B5C52] border border-[#9B5C52]/30 text-[10px] font-bold uppercase tracking-wider">
            Open Dispute
          </span>
        );
      case 'UNDER_REVIEW':
        return (
          <span className="px-3 py-1 rounded-full bg-[#C8A46A]/20 text-[#8B6A4F] border border-[#C8A46A]/40 text-[10px] font-bold uppercase tracking-wider">
            Under Review
          </span>
        );
      case 'RESOLVED_BUYER_REFUND':
        return (
          <span className="px-3 py-1 rounded-full bg-[#6E8A62]/15 text-[#6E8A62] border border-[#6E8A62]/30 text-[10px] font-bold uppercase tracking-wider">
            Buyer Refunded
          </span>
        );
      case 'RESOLVED_SELLER_PAYOUT':
        return (
          <span className="px-3 py-1 rounded-full bg-[#6E8A62]/15 text-[#6E8A62] border border-[#6E8A62]/30 text-[10px] font-bold uppercase tracking-wider">
            Seller Paid
          </span>
        );
      case 'REJECTED':
        return (
          <span className="px-3 py-1 rounded-full bg-[#8B7562]/20 text-[#6E5948] border border-[#8B7562]/30 text-[10px] font-bold uppercase tracking-wider">
            Rejected
          </span>
        );
      default:
        return (
          <span className="px-3 py-1 rounded-full bg-[#EDE5D9] text-[#3B2A22] border border-[#D6C8B8] text-[10px] font-bold uppercase tracking-wider">
            {status}
          </span>
        );
    }
  };

  const filterTabs = [
    { key: 'ALL', label: 'All Disputes' },
    { key: 'OPENED', label: 'Open' },
    { key: 'UNDER_REVIEW', label: 'Under Review' },
    { key: 'RESOLVED_BUYER_REFUND', label: 'Refunded' },
    { key: 'RESOLVED_SELLER_PAYOUT', label: 'Seller Payout' },
    { key: 'REJECTED', label: 'Rejected' },
  ];

  return (
    <div className="space-y-8 text-[#3B2A22]">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <span className="tag-editorial mb-2 block">Resolution Center</span>
          <h1 className="font-heading text-4xl font-normal text-[#3B2A22]">Disputes & Refunds</h1>
          <p className="font-sans text-xs text-[#8B7562] mt-1">
            Arbitrate campus escrow transactions, missing handshakes, and condition disputes
          </p>
        </div>
      </div>

      {actionSuccess && (
        <div className="p-4 rounded-2xl bg-[#6E8A62]/15 border border-[#6E8A62]/30 text-[#6E8A62] text-xs font-sans font-semibold flex items-center justify-between">
          <span>✓ {actionSuccess}</span>
          <button onClick={() => setActionSuccess(null)} className="hover:opacity-75">✕</button>
        </div>
      )}

      {/* Filter Tabs */}
      <div className="flex flex-wrap gap-2 border-b border-[#D6C8B8] pb-4">
        {filterTabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setSelectedStatus(tab.key)}
            className={`px-4 py-2 rounded-full font-sans text-xs font-semibold transition-all ${
              selectedStatus === tab.key
                ? 'bg-[#111111] text-[#F4EFE7]'
                : 'bg-[#EDE5D9] text-[#6E5948] hover:bg-[#E7DED1] hover:text-[#3B2A22] border border-[#D6C8B8]'
            }`}
          >
            {tab.label}
          </button>
        ))}
      </div>

      {isLoading ? (
        <div className="text-center py-20 text-xs text-[#8B7562]">
          <div className="w-10 h-10 rounded-full border-2 border-[#C8A46A] border-t-transparent animate-spin mx-auto mb-3" />
          Loading dispute resolution queue…
        </div>
      ) : disputes.length === 0 ? (
        <div className="text-center py-20 px-6 rounded-[32px] bg-[#EDE5D9] border border-[#D6C8B8] shadow-warm-subtle space-y-3">
          <div className="w-14 h-14 rounded-2xl bg-[#E7DED1] text-[#3B2A22] flex items-center justify-center mx-auto">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#C8A46A]">
              <path d="M12 3v18M3 7h18M6 12l-3 7h6l-3-7zM18 12l-3 7h6l-3-7z" />
            </svg>
          </div>
          <h3 className="font-heading text-3xl font-normal text-[#3B2A22]">No Disputes Found</h3>
          <p className="font-sans text-xs text-[#8B7562] max-w-sm mx-auto">
            {selectedStatus === 'ALL'
              ? 'All campus transaction handshakes have been completed cleanly.'
              : `No disputes found matching filter "${selectedStatus}".`}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {disputes.map((d) => (
            <div
              key={d.id}
              className="p-6 sm:p-8 rounded-[32px] bg-[#EDE5D9] border border-[#D6C8B8] shadow-warm-subtle space-y-5"
            >
              {/* Top Row: Dispute ID, Reason & Status */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#D6C8B8] pb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    <span className="font-heading text-2xl font-normal text-[#3B2A22]">
                      Dispute #{d.id.slice(0, 8)}
                    </span>
                    {getStatusBadge(d.status)}
                  </div>
                  <p className="font-sans text-xs text-[#8B6A4F] font-semibold">
                    Reason: {d.reason.replace(/_/g, ' ')}
                  </p>
                </div>

                <div className="flex items-center gap-3 font-sans text-xs">
                  <span className="font-heading text-2xl font-normal text-[#3B2A22]">
                    {formatINR(d.order?.totalAmount || 0)}
                  </span>
                  <button
                    onClick={() => handleOpenArbitration(d)}
                    className="btn-primary text-xs !py-2 !px-5"
                  >
                    Arbitrate Case
                  </button>
                </div>
              </div>

              {/* Middle Row: Parties & Order Info */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 font-sans text-xs bg-[#E7DED1] p-4 rounded-2xl border border-[#D6C8B8]">
                <div>
                  <span className="text-[10px] font-semibold text-[#8B7562] uppercase block">Order Record</span>
                  <Link
                    to={`/orders/${d.order?.orderNumber}`}
                    className="font-mono text-xs font-semibold text-[#3B2A22] hover:underline block truncate"
                  >
                    #{d.order?.orderNumber}
                  </Link>
                  <span className="text-[10px] text-[#8B7562]">
                    {new Date(d.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </span>
                </div>

                <div>
                  <span className="text-[10px] font-semibold text-[#8B7562] uppercase block">Buyer (Initiator)</span>
                  <span className="font-semibold text-[#3B2A22] block truncate">
                    {d.order?.buyer?.firstName} {d.order?.buyer?.lastName}
                  </span>
                  <span className="text-[11px] text-[#8B7562] block truncate">{d.order?.buyer?.email}</span>
                </div>

                <div>
                  <span className="text-[10px] font-semibold text-[#8B7562] uppercase block">Seller Storefront</span>
                  <span className="font-semibold text-[#3B2A22] block truncate">
                    {d.order?.seller?.storeName || 'Storefront'}
                  </span>
                  <span className="text-[11px] text-[#8B7562] block truncate">
                    {d.order?.seller?.user?.email || 'N/A'}
                  </span>
                </div>
              </div>

              {/* Explanation & Notes */}
              <div className="font-sans text-xs space-y-2">
                <p className="text-[#4A392F] leading-relaxed">
                  <strong className="text-[#3B2A22]">Dispute Explanation:</strong> {d.explanation}
                </p>

                {d.resolutionNotes && (
                  <div className="p-3.5 rounded-2xl bg-[#F4EFE7] border border-[#D6C8B8] text-xs">
                    <span className="font-semibold text-[#3B2A22] block mb-0.5">Admin Resolution Decision:</span>
                    <p className="text-[#6E5948]">{d.resolutionNotes}</p>
                    {d.resolvedAt && (
                      <span className="text-[10px] text-[#8B7562] mt-1 block">
                        Resolved on {new Date(d.resolvedAt).toLocaleDateString('en-IN')}
                      </span>
                    )}
                  </div>
                )}
              </div>
            </div>
          ))}
        </div>
      )}

      {/* Arbitration Modal */}
      {activeDispute && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-xl rounded-[32px] bg-[#EDE5D9] border border-[#D6C8B8] shadow-2xl p-6 sm:p-8 space-y-6 text-[#3B2A22] max-h-[90vh] overflow-y-auto">
            
            <div className="flex items-start justify-between border-b border-[#D6C8B8] pb-4">
              <div>
                <span className="tag-editorial mb-1 block">Admin Arbitration</span>
                <h2 className="font-heading text-2xl font-normal text-[#3B2A22]">
                  Arbitrate Dispute #{activeDispute.id.slice(0, 8)}
                </h2>
                <p className="font-sans text-xs text-[#8B7562] mt-0.5">
                  Order #{activeDispute.order?.orderNumber} • {formatINR(activeDispute.order?.totalAmount || 0)}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setActiveDispute(null)}
                className="w-8 h-8 rounded-full bg-[#E7DED1] hover:bg-[#D6C8B8] text-[#8B7562] hover:text-[#3B2A22] flex items-center justify-center transition-colors text-sm"
              >
                ✕
              </button>
            </div>

            {actionError && (
              <div className="p-3.5 rounded-2xl bg-[#9B5C52]/15 border border-[#9B5C52]/30 text-[#9B5C52] text-xs font-sans font-semibold">
                {actionError}
              </div>
            )}

            <div className="p-4 rounded-2xl bg-[#E7DED1] border border-[#D6C8B8] font-sans text-xs space-y-2">
              <p>
                <strong className="text-[#3B2A22]">Reason:</strong> {activeDispute.reason.replace(/_/g, ' ')}
              </p>
              <p className="text-[#6E5948]">
                <strong className="text-[#3B2A22]">Buyer's Claim:</strong> {activeDispute.explanation}
              </p>
            </div>

            <form onSubmit={handleResolveDispute} className="space-y-4 font-sans text-xs">
              <div>
                <label className="font-sans text-[10px] tracking-[0.15em] uppercase font-semibold text-[#8B7562] block mb-2">
                  Arbitration Decision / Resolution Action
                </label>
                <select
                  value={resolutionStatus}
                  onChange={(e) => setResolutionStatus(e.target.value as any)}
                  className="input-editorial cursor-pointer"
                >
                  <option value="RESOLVED_BUYER_REFUND">
                    ✓ Refund Buyer (Return Escrow to Buyer & Mark Order Refunded)
                  </option>
                  <option value="RESOLVED_SELLER_PAYOUT">
                    ✓ Release Payout to Seller (Release Escrow & Mark Order Completed)
                  </option>
                  <option value="UNDER_REVIEW">
                    ⏳ Mark Under Investigation / Pending Review
                  </option>
                  <option value="REJECTED">
                    ✕ Reject Dispute (Close Dispute without Refund)
                  </option>
                </select>
              </div>

              <div>
                <label className="font-sans text-[10px] tracking-[0.15em] uppercase font-semibold text-[#8B7562] block mb-2">
                  Resolution Notes / Reason Sent to Parties
                </label>
                <textarea
                  rows={4}
                  value={resolutionNotes}
                  onChange={(e) => setResolutionNotes(e.target.value)}
                  placeholder="Provide explanation of decision for both buyer and seller (e.g. Buyer submitted photo proof of extensive defect; full refund approved)..."
                  className="input-editorial !py-3 resize-none"
                />
              </div>

              <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#D6C8B8]">
                <button
                  type="button"
                  onClick={() => setActiveDispute(null)}
                  disabled={isSubmittingResolution}
                  className="btn-secondary !py-2.5 !px-5 text-xs"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isSubmittingResolution}
                  className="btn-primary !py-2.5 !px-6 text-xs flex items-center gap-2"
                >
                  {isSubmittingResolution ? 'Applying Resolution…' : 'Confirm Resolution'}
                </button>
              </div>
            </form>

          </div>
        </div>
      )}

    </div>
  );
};
